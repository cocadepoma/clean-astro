...
const useFrameworks = (getFrameworksUseCase, getFrameworkByNameUseCase) => {
  const [frameworks, setFrameworks] = createSignal([]);
  const [framework, setFramework] = createSignal(null);
  ...
  const fetchFrameworkByName = async () => {
    try {
      setLoading(true);
      const framework2 = await getFrameworkByNameUseCase.execute("Solid");
